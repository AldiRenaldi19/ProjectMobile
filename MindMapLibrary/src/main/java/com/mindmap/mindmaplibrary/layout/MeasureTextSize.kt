...
class MeasureTextSize(private val context: Context) {
    ...
    private fun changeSize(
        ...
        is CircleNodeData -> {
            val newRadius =
                Dp(
                    maxOf(
                        (Dp(Px(width).toDp(context) / 2f) + drawInfo.padding.dpVal).dpVal,
                        ((Dp(Px(height).toDp(context)) + drawInfo.padding * 2) / 2f).dpVal,
                    ),
                )
            nodeData.copy(
                id = nodeData.id,
        ...
        is RectangleNodeData -> {
            var newWidth = Dp(Dp(Px(width).toDp(context)).dpVal + drawInfo.padding.dpVal)
    ...
    private fun sumTotalHeight(description: String): Float {
        ...
        description.split("\n").forEach { line ->
            ...
            sum += bounds.height().toFloat() + drawInfo.lineHeight.dpVal
        }
        return sum
    }
